...
class UId3(BaseEstimator):
    ...
    @staticmethod
    def fit(data, entropyEvaluator, depth: int):
        ...
        for val in best_split.get_splittable_domain():
            if best_split.get_type() == Attribute.TYPE_NOMINAL:
                ...
                subtree = UId3.fit(new_data, EntropyEvaluator, depth + 1)
                ...
                subtree_less_than = UId3.fit(new_data_less_then, EntropyEvaluator, depth + 1)
                subtree_greater_equal = UId3.fit(new_data_greater_equal, EntropyEvaluator, depth + 1)
                best_split_stats = data.calculate_statistics(best_split)
    ...
    def fit_uncertain_nominal() -> None:
        data = Data.parse_uarff("/Users/ola/Develop/udt/resources/machine.nominal.uncertain.arff")
        test = Data.parse_uarff("/Users/ola/Develop/udt/resources/machine.nominal.uncertain.arff")
        ...
        t.save_dot("/Users/ola/Develop/pyuid3/tree.dot")
        t.save_HML("/Users/ola/Develop/pyuid3/tree-n.hml")
        t.save_HMR("/Users/ola/Develop/pyuid3/tree.hmr")

        # For eval train-hoeffding
        for val in data.get_attribute_of_name("hour").get_domain():
            prt = f"{val},"
            nr =  Reading.parse_reading(data.get_attribute_of_name("hour"), val)
            reads = [nr]
            vv = t.predict(Instance(reads)).get_most_probable()
            prt += f"{vv.get_name()},{vv.get_confidence()}"
            print(prt)
